		...
		.execute_with(|| {
			...
			pretty_assertions::assert_eq!(System::events(), vec![
				EventRecord {
					...
					event: MetaEvent::system(frame_system::RawEvent::NewAccount(addr)),
					topics: vec![],
				},
				EventRecord {
					phase: Phase::Initialization,
...
fn deduct_blocks() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(Contracts::call(Origin::signed(ALICE), addr, 0, GAS_LIMIT, call::null()));
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(System::events(), vec![
				EventRecord {
					phase: Phase::Initialization,
					event: MetaEvent::contracts(RawEvent::Evicted(addr, true)),
					topics: vec![],
				},
			]);
...
fn restoration(test_different_storage: bool, test_restore_to_with_dirty_storage: bool) {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_err_ignore_postinfo!(
				Contracts::call(Origin::signed(ALICE), addr_bob, 0, GAS_LIMIT, call::null()),
				Error::<Test>::NotCallable
			);
			...
			assert_eq!(System::events(), vec![
				EventRecord {
					...
					event: MetaEvent::contracts(
						RawEvent::Evicted(addr_bob, true)
					),
					topics: vec![],
				},
			]);
			...
			if test_different_storage || test_restore_to_with_dirty_storage {
				...
				match (test_different_storage, test_restore_to_with_dirty_storage) {
					...
					(_, true) => {
						pretty_assertions::assert_eq!(System::events(), vec![
							...
							EventRecord {
								phase: Phase::Initialization,
								event: MetaEvent::system(frame_system::RawEvent::NewAccount(addr_django)),
								topics: vec![],
							},
							EventRecord {
								phase: Phase::Initialization,
								event: MetaEvent::balances(pallet_balances::RawEvent::Endowed(addr_django, 30_000)),
								topics: vec![],
							},
							EventRecord {
								phase: Phase::Initialization,
								event: MetaEvent::balances(
									pallet_balances::RawEvent::Transfer(CHARLIE, addr_django, 30_000)
								),
								topics: vec![],
							},
							EventRecord {
								phase: Phase::Initialization,
								event: MetaEvent::contracts(RawEvent::Instantiated(CHARLIE, addr_django)),
								topics: vec![],
							},
						]);
					}
				...
				assert_eq!(System::events(), vec![
					EventRecord {
						phase: Phase::Initialization,
						event: MetaEvent::system(system::RawEvent::KilledAccount(addr_django)),
						topics: vec![],
					},
					EventRecord {
						phase: Phase::Initialization,
